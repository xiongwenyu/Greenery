...
using Greenery.MessageQueueMiddleware.Exceptions;
using log4net;
...
namespace Greenery.MessageQueueMiddleware.Implementations
{
    ...
    public class MessageQueueAgent : IMessageQueueAgent
    {
        ...
        /// 消息队列适配器模块名称
        /// </summary>
        private static readonly string MESSAGEQUEUEASSEMBLY = "MessageQueue.dll";
        /// <summary>
        /// 消息队列代理器存储模块名称
        /// </summary>
        private static readonly string MESSAGEQUEUEAGENTSTORAGE = "MessageQueueAgentStorage.dll";
        ...
        public MessageQueueAgent(MQMServer server)
        {
            ...
            var dllFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, MESSAGEQUEUEASSEMBLY);
            Adapter = dllFile.GetImplementedObjectsByInterface<IMessageQueueAdapter>().FirstOrDefault();
            if (Adapter == null)
            {
                throw new TypeLoadException("未能正确加载消息队列适配器！");
            }

            dllFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, MESSAGEQUEUEAGENTSTORAGE);
            Storage = dllFile.GetImplementedObjectsByInterface<IMQMStorage>().FirstOrDefault();

            if (Storage == null)
            {
                throw new TypeLoadException("未能正确加载存储模块！");
            }
            ...
            {
                var group = item.Descriptions.FirstOrDefault();
        ...
        public void Subscribe(Guid publisherId, SubscribeItem subscribeItem)
        {
            ...
            var group = subscribeItem.Descriptions.FirstOrDefault();
            if (string.IsNullOrEmpty(group))
            {
                throw new Exception();
            }
        ...
        public void UnSubscribe(Guid publisherId, SubscribeItem subscribeItem)
        {
            ...
            if (Storage.TryGetClientIndetity(publisherId, out subscriber))
            {
                ...
                if (Storage.TryGetSubscribeItem(publisherId, subscribeItem.Descriptions, subscribeItem.FilterMode, out old))
                {
                    ...
                    if (!Storage.HasSubscribers(AgentId, new string[] { group }, true))
                    {
